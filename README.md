...
2. Add your `worldnewsapi.com` key as `WORLD_NEWS_API_KEY=...`
...
- `Assembly` shows live legislative coverage from World News API, filtered toward Tamil Nadu Legislative Assembly proceedings, governance, public policy, and manifesto-related debate
...
- Use `Vercel`, not `GitHub Pages`, because this project needs a server for `/api/assembly-news`, `/api/blogs`, `/api/admin/login`, and it must keep `WORLD_NEWS_API_KEY` and `BLOG_ADMIN_PASSWORD` secret
...
- On Vercel, deploy the repo, then add environment variables for `WORLD_NEWS_API_KEY`, `BLOG_ADMIN_PASSWORD`, and optionally `ADMIN_TOKEN_SECRET`
